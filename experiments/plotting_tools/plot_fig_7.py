...
def parse_debug_wq(log):
    ...
    for line in lines:
        if 'tx to' in line and ' put ' in line:
...
def parse_txn_wq(log):
    task_info = {}
    worker_info = {}
    library_info = {}
    file_info = {}
    file_sizes = {}
    manager_info = {'type':'txn', 'log':log, 'start':0, 'stop':0, 'first_task':float('inf'), 'last_task_done':float('-inf'), 'input_transfers':[]}

    # parse relevant info
    lines = open(log, 'r').read().splitlines()
    for line in lines:
        try:
            (time, m_pid, category, obj, info) = line.split(maxsplit=4)
            time = float(time)/MICROSECONDS
        except:
            continue
        if category == 'TRANSFER':
            task_id, flag, size_in_mb, walltime, filename = info.split(maxsplit=4)
            if obj =='INPUT':
               manager_info['input_transfers'].append([time, float(size_in_mb)*1000000]) 
            if obj =='OUTPUT':
                pass
        if category == 'TASK':
            if obj not in task_info:
                task_info[obj] = {}
            try:
                status, worker_cat, info = info.split(maxsplit=2)
            except:
                status, worker_cat = info.split(maxsplit=1)
            if status == 'WAITING': 
                task_info[obj]['function'] = worker_cat
                task_info[obj]['ready'] = time
                task_info[obj]['id'] = obj
            if status == 'RUNNING':
                task_info[obj]['running'] = time
                task_info[obj]['worker'] = worker_cat
        ...
        if category == 'WORKER':
            try:
                obj, status, info = info.split(maxsplit=2)
            except:
                obj, status= info.split(maxsplit=1)
    ...
    for line in lines:
        ...
        if category == 'WORKER':
            if obj not in worker_info:
                worker_info[obj] = {'tasks':[], 'libraries':[], 'cache':{}}
            if status == 'TRANSFER':
                cat, filename, size, wall_time, start_time = info.split(maxsplit=4)
                size = float(size)
                if cat == 'INPUT':
                    manager_info['input_transfers'].append([time, size])
                if cat == 'OUTPUT':
                    pass
            if status == 'CONNECTION':
                worker_info[obj]['start'] = time
...
def plot_worker(log_info, axs, args, color, ylim, legend):
    ...
    for worker in worker_info:
        ...
        for slot in slots:
            for task in slots[slot]:
                task_running_info['ys'].append(y_base)
        ...
        y_count += cores # Could get resources from txn log
        y_counts.append(y_count)

        
    if(task_running_info['ys']):
        axs.barh(task_running_info['ys'], task_running_info['widths'], left=task_running_info['lefts'], label=legend, height=-.8, align='edge', color=color)
    
    # trickery for y axis ticks
    tick_count = args.worker_ticks
    tt = len(y_counts)
    if legend == 'Function Calls (Stack 4)':
        tt = 200
    steps = int(tt/tick_count) 
    y_labels = []
    y_ticks = [y_counts[x] for x in range(steps - 1, tt, steps) if y_counts[steps - 1]]
    ...
    axs.set_xlabel('Application Runtime (s)', fontsize=12)
    axs.set_ylabel('Workers', fontsize=12)
    axs.set_yticks(y_ticks)
    axs.set_yticklabels(y_labels, fontsize=12)
    axs.tick_params(axis='both', labelsize=12)
    #axs.set_xticks(fontsize=12)
    axs.set_xlim(left=0)
    axs.set_xlim(right=800)
    axs.set_ylim(top=ylim*cores)
    if legend:
...
def get_input_trans(log_info):
    manager_info = log_info['manager_info']
    origin = 0
    first_task =  log_info['manager_info']['first_task']
    manager_start = log_info['manager_info']['start']
    if args.origin == 'first-task':
    ...
    trans_info = {'gb_sent':0, 'x':[], 'y':[]}
    for input_transfer in manager_info['input_transfers']:
        time = input_transfer[0]
        data_sent = input_transfer[1]
        trans_info['gb_sent'] += data_sent
        trans_info['y'].append(trans_info['gb_sent'])
        trans_info['x'].append(time - origin)

    return trans_info

def get_state(log_info, return_state):
    ...
    return state_info[return_state]
...
def plot_data_exg(log_info, axs, args, base):
    ...
    im = None
    if not base:
        im = axs.pcolormesh(C, norm='log', cmap='cividis', hatch='/')
        axs.set_title('No Peer Transfers', fontsize=12)
        axs.set_ylabel('Source(ID)', fontsize=12)
        axs.set_yticks([0.5, 5.5, 10.5, 15.5, 20.5])
        axs.set_yticklabels([0, 5, 10, 15, 20], fontsize=12)
    if base:
        axs.pcolormesh(C, norm=base.norm, cmap='cividis')
        axs.set_title('Peer Transfers', fontsize=12)
        fig = axs.get_figure()
        cbar = fig.colorbar(base, ax=axs)
        cbar.ax.set_ylabel("Transfered Data (GB)", fontsize=12)
    
    axs.set_xticks([0.5, 5.5, 10.5, 15.5, 20.5])
    axs.set_xticklabels([0, 5, 10, 15, 20], fontsize=12)
    axs.set_xlabel('Destination(ID)', fontsize=12)
    return im
...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(1, 2, squeeze=False, sharey=True, gridspec_kw={'wspace':.1})
    log_info = parse_debug_wq('logs/wq_4j_run-1_db')
    im1 = plot_data_exg(log_info, axs[0][0], args, None)
    log_info = parse_debug('logs/tv_tasks_1j_run-1_db')
    im2 = plot_data_exg(log_info, axs[0][1], args, im1)
    fig.set_size_inches(w=6.4*2, h=4.8)
    fig.set_dpi(300)
    fig.savefig('graphs/DV3DataExg.pdf')
    fig.savefig('graphs/DV3DataExg.png')
