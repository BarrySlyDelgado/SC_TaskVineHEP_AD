...
def parse_txn(log):
    ...
    file_info = {}
    file_sizes = {}
    manager_info = {'type':'txn', 'log':log, 'start':0, 'stop':0, 'first_task':float('inf'), 'last_task_done':float('-inf'),  'connected_workers':[]}
    ...
    for line in lines:
        ...
        if category == 'WORKER':
            ...
            if status == 'CONNECTION':
                worker_info[obj]['start'] = time
                manager_info['connected_workers'].append([time, 'connection'])
            if status == 'DISCONNECTION':
                worker_info[obj]['stop'] = time
                manager_info['connected_workers'].append([time, 'disconnection'])
            if status == 'CACHE_UPDATE':
                (filename, size, wall_time, start_time) = info.split()
                size = float(size)
                if filename not in file_sizes:
...
def get_worker_con(log_info, return_state):
    ...
    if args.origin == 'first-task':
        ...
        origin = manager_start
    info = {'count':0, 'x':[], 'y':[]}
    for com in manager_info['connected_workers']:
        com_type = com[1]
        time = com[0]
        if com_type == 'connection':
            info['count'] += 1
            info['y'].append(info['count'])
            info['x'].append(time - origin)
        else:
            info['count'] -= 1
            info['y'].append(info['count'])
            info['x'].append(time - origin)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--origin', nargs='?', default='manager-start', choices=['first-task', 'manager-start'], type=str)
    ...
    fig, axs = plt.subplots(1, 1, squeeze=False, figsize=[6.4, 2.4], sharex=True, layout='constrained')
    fig.subplots_adjust(hspace=0.05)

    log_info = parse_txn('logs/tv_fcs_4j_run-1_tr')
    running_info_1 = get_state(log_info, 'running')
    print('1')


    axs[0][0].plot(running_info_1['x'], running_info_1['y'], linestyle='-', label='Running Tasks', color='darkgreen')


    #ax.set_xscale('log')
    axs[0][0].tick_params(labelsize=12)
    ax=axs[0][0]
    ax.set_xlabel('Aplication Runtime (s)', fontsize=12)
    ax.set_ylabel('Concurrent Tasks (s)', fontsize=12)
    plt.yticks(fontsize=12)
    plt.xticks(fontsize=12)
    fig.set_dpi(300)
    fig.savefig('graphs/DV3HeroRun.pdf')
