...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(2, 2, squeeze=False, sharex=True, gridspec_kw={'hspace':0})
    ax = axs[0][0]
    log_info = parse_txn('logs/tv_tasks_1j_run-1_tr')
    plot_worker(log_info, ax, args, 'darkblue', 1, None)
    ax.set_title('20 Workers')
    ax = axs[0][1]
    log_info = parse_txn('logs/tv_tasks_4j_run-1_tr')
    plot_worker(log_info, ax, args, 'darkblue', 4, 'Tasks (Stack 3)')
    ax.set_title('200 Workers')

    ax = axs[1][0]
    log_info = parse_txn('logs/tv_fcs_1j_run-1_tr')
    plot_worker(log_info, ax, args, 'darkgreen', 1, None)
    ax = axs[1][1]
    log_info = parse_txn('logs/tv_fcs_4j_run-1_tr')
    plot_worker(log_info, ax, args, 'darkgreen', 4, 'Function Calls (Stack 4)')
    fig.set_dpi(300)
    width = 6.4 * 2
    height = 4.8 * 2
    fig.set_size_inches(w=width, h=height)
    fig.savefig('graphs/DV3_scale_half_column.pdf')
