...
def plot_runtime(log_info, axs, args, label, color, linestyle):    
    ...
    axs.plot(runtime, p_complete, label=label, color=color, linestyle=linestyle)
    axs.set_xscale('log')
...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(1, 1, squeeze=False, figsize=[6.4, 2.4], layout='constrained')
    #width = 6.4 * 1
    #height = (4.8 * 1) / 2
    #fig.set_size_inches(w=width, h=height)
    count = 0
    plt.xlabel('Task Runtime (s)', fontsize=12)
    plt.ylabel('Percent of Tasks', fontsize=12)
    log_info = parse_txn('logs/tv_tasks_1j_run-1_tr')
    plot_runtime(log_info, axs[0][0], args, 'Tasks (Stack 3)', color='darkblue', linestyle='-.')
    log_info = parse_txn('logs/tv_fcs_1j_run-1_tr')
    plot_runtime(log_info, axs[0][0], args, 'Function Calls (Stack 4)', color='darkgreen', linestyle=':')
    fig.set_dpi(300)
    fig.savefig('graphs/DV3ExecutionParadigmRuntimeSmall.pdf')
    fig.savefig('graphs/DV3ExecutionParadigmRuntimeSmall.png')
