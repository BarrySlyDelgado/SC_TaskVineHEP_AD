...
def plot_cache(log_info, axs, args, words=[]):
    ...
    for worker in worker_info:
        ...
        if 'stop' in worker_info[worker]:
            for time in worker_info[worker]['stop']:
                ...
                dy.append(gb_count)
    axs.plot(dx,dy, 'rx', label='Worker Failure')
    axs.plot(tx,ty, 'b4', label='Scheduled Merging Task')
def plot_worker(log_info, axs, args):
    ...
    for worker in worker_info:
        ...
        for task in worker_info[worker]['tasks']:
            try:
                tasks.append([task['running'], task['waiting_retrieval'], task['retrieved']])
                if task['waiting_retrieval'] - task['running'] > 500:
                        print('LONG RUNNING TASK', worker, task['function'])
            except:
...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(nrows, ncols, squeeze=False, sharey=True, layout='constrained')
    #ax = axs[0][0]
    #log_info = parse_txn('logs/original_tr')
    #plot_cache(log_info, ax, args, words=["compute"])
    #ax.set_xlabel('Aplication Runtime (s)', fontsize=12)
    #ax.set_ylabel('Worker Cache Usage (GB)', fontsize=12)
    #ax.set_xlim(right=6000)
    #ax.tick_params(which='both', labelsize=12)
    ax = axs[0][0]
    log_info = parse_txn('logs/rst_1j_run-1_tr')
    plot_cache(log_info, ax, args, words=["partition", "apply"])
    ax.set_xlabel('Aplication Runtime (s)', fontsize=12)
    ...
    fig.savefig("graphs/RSTri-CacheGraphMergeUpdated.pdf")
