...
if __name__ == '__main__':
    ...
    ax = axs[0][0]
    log_info = parse_txn('logs/tasks_20w_tr')
    plot_worker(log_info, ax, args, 'darkblue', 20, None)
    ax.set_title('20 Workers')
    ax = axs[0][1]
    log_info = parse_txn('logs/tasks_200w_tr')
    plot_worker(log_info, ax, args, 'darkblue', 200, 'Tasks (Stack 3)')
    ax.set_title('200 Workers')

    ax = axs[1][0]
    log_info = parse_txn('logs/fcs_20w_tr')
    plot_worker(log_info, ax, args, 'darkgreen', 20, None)
    ax = axs[1][1]
    log_info = parse_txn('logs/fcs_200w_tr')
    plot_worker(log_info, ax, args, 'darkgreen', 200, 'Function Calls (Stack 4)')
    fig.set_dpi(300)
    width = 6.4 * 2
    height = 4.8 * 2
