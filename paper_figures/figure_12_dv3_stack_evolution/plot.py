...
def parse_txn_wq(log):
    ...
    for line in lines:
        try:
            ...
            try:
                (time, m_pid, category, info) = line.split(maxsplit=3)
                time = float(time)/MICROSECONDS
            except:
                continue
...
def get_state(log_info, return_state):
    ...
    origin = 0
    first_task =  log_info['manager_info']['first_task'] 
    last_task =  log_info['manager_info']['last_task_done'] 
    manager_start = log_info['manager_info']['start']
    print(manager_start)
    if args.origin == 'first-task':
        origin = first_task
...
def get_input_trans(log_info):
    ...
    for input_transfer in manager_info['input_transfers']:
        ...
        trans_info['gb_sent'] += data_sent/1000000000
...
if __name__ == '__main__':
    ...
    d = .5
    fig, axs = plt.subplots(2, 1, squeeze=False, sharex='col', sharey='row')
    ...
    log_info_1 = parse_txn_wq('logs/stack_1_tr')
    log_info_2 = parse_txn_wq('logs/stack_2_tr')
    log_info_3 = parse_txn('logs/stack_3_tr')
    log_info_4 = parse_txn('logs/stack_4_tr')

    ax = axs[0][0]
    #ax1= axs[0][1]
    state = 'running'
    running_info_1 = get_state(log_info_1, state)
    running_info_2 = get_state(log_info_2, state)
    running_info_3 = get_state(log_info_3, state)
    running_info_4 = get_state(log_info_4, state)
    ax.plot(running_info_1['x'], running_info_1['y'], linestyle='-', label='Stack 1', color='black')
    ax.plot(running_info_2['x'], running_info_2['y'], linestyle='--', label='Stack 2', color='black')
    ax.plot(running_info_3['x'], running_info_3['y'], linestyle='-.', label='Stack 3', color='black')
    ax.plot(running_info_4['x'], running_info_4['y'], linestyle=':', label='Stack 4', color ='black')
    ...
    ax.plot(running_info_4['x'], running_info_4['y'], linestyle=':', label='Stack 4', color ='black')
    ax.set_xlim(left=0)
    ax.set_ylabel('Waiting Tasks')
    ax.set_xlim(right=300)
    ax.set_xticks([0, 100, 200, 300])
    fig.supxlabel('Application Runtime (s)', fontsize=12)
    fig.savefig('graphs/DV3ProgressionStacked.pdf')
